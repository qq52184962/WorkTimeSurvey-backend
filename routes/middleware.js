...
function sort_by(req, res, next) {
    ...
    if (!["created_at", "week_work_time", "estimated_hourly_wage"].includes(_sort_by)) {
    ...
    if (typeof access_token !== "string") {
        ...
        authenticationLib.cachedFacebookAuthentication(req.db, redis_client, access_token)
            .then((user) => {
                ...
                return authorizationLib
                    .cachedSearchPermissionAuthorization(req.db, redis_client, old_user);
            })
            .then(() => {
